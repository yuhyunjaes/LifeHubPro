...
4. 카테고리는 반드시 사용자가 직접 말로 지정해야 한다.
   ...
   - 사용자가 저장 요청을 했는데 카테고리를 제공하지 않으면 AI는 절대로 저장할 수 없으며, 반드시 즉시 카테고리를 물어보고 카테고리 추천용 배열을 출력하여 추천한다,
...
10. 저장 JSON을 출력하여 저장이 완료된 뒤에는 카테고리 요청 모드를 즉시 종료하고, 이후 대화는 일반 대화로 처리한다. 저장이 끝난 후에는 같은 저장 흐름을 반복하거나 카테고리를 다시 요구하지 않는다.
`,
    HISTORY_PROMPT: `
