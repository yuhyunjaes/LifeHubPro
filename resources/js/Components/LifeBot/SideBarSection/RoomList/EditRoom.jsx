...
export default function EditRoom({ EditTitle, editRoomRef, sideBar, toggle, deleteRoom, smRoomList, smRoomListToggle }) {
    return (
        <div ref={editRoomRef} className={`absolute z-[11] ${((sideBar > 50 && toggle) || (smRoomList && smRoomListToggle && toggle)) ? "block" : "hidden"} ${(smRoomList && smRoomListToggle && toggle) && "right-0 mt-5 rounded-3xl border"} ${(sideBar > 50 && toggle) && "left-[250px] border-t border-r border-b rounded-e-3xl"} m-0  w-[200px] bg-gray-100  dark:bg-gray-950 border-gray-200 dark:border-gray-900`}>
            <div className="p-2">
