...
export default function NoteInsightSection() {
    ...
    const getCategories = useCallback(async () => {
        try {
            const res = await axios.get<CategoriesResponse>("/api/notepads/categories");
    ...
    const bottomText = useMemo(() => {
        ...
        if (topCategories.length === 1) {
            return `가장 많이 작성한 카테고리는 '${topCategories[0] && topCategories[0].category}' 입니다.`;
        } else {
    ...
    return (
        <div className="card border row-span-2 border-gray-300 dark:border-gray-800 col-span-2 lg:col-span-2 p-3 flex flex-col">
            <h3 className="text-sm normal-text font-semibold mb-3">메모장 카테고리 비율</h3>

            <div id="chart-category" className="relative flex-3 w-full h-full min-h-[200px]"></div>
